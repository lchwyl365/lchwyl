...
public class ${model.className}ServiceImpl implements ${model.className}Service {
	...
	public ${model.className} searchById(Long tbid){
		return ${model.className?uncap_first}Mapper.selectByPrimaryKey(tbid);
	}
	...
	public int delete(Long tbid){
		return ${model.className?uncap_first}Mapper.deleteByPrimaryKey(tbid);
	}
