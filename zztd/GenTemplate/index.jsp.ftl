<#macro fieldTh fields>
	<#list fields as field>
					<th>${field.name}</th>
	</#list>
</#macro>
<#macro fieldTd fields>
	<#list fields as field>
					<td>${r"$"}{${field.name}}</td>
	</#list>
</#macro>
   ...
   <div class="container" style="margin-top:30px;">
    	<fieldset>
		    ...
		    <table class="table table-hover">
              <thead>
                <tr>
                  <@fieldTh fields=model.fields/>
                </tr>
              </thead>
              <tbody>
              	<s:iterator var="item" value="#request.pager.datas">
                	<tr>
                  		<@fieldTd fields=model.fields/>
	                  	<td>
            ...
            <s:if test="%{#request.pager.total > 0}">
					...
					<div id="pagination" class="pagination" >
						<ul>
							<pg:pager url="${r"${contextPath}"}/front/${model.className?uncap_first}_index.action" items="${r"${pager.total}"}"	
...
$(function(){
	$("#add-btn").click(function(){
		window.self.location = "${r"${contextPath}"}/${model.className?uncap_first}/add.jsp";
	});
	$(".button-flat-highlight").click(function(){
		...
		$.dialog({
		    ...
		    ok: function () {
		    	/**确认信息*/
		    	$.post("${r"${contextPath}"}/front/${model.className?uncap_first}_delete.action",{
	...
	$(".button-flat-primary").click(function(){
		var tbid = $(this).attr("tbid");
		window.self.location = "${r"${contextPath}"}/front/${model.className?uncap_first}_intoUpdate.action?${model.className?uncap_first}.tbid="+tbid;
	});
});
//刷新当前页面
function refresh(){
	window.self.location = "${r"${contextPath}"}/front/${model.className?uncap_first}_index.action?pager.offset=${r"${pager.offset}"}";
}
</script>
</body>
</html>