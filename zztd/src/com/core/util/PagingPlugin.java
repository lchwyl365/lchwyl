...
public class PagingPlugin implements Interceptor {
	...
	private long getTotal(Invocation invocation) throws Exception{
        ...
        String countSql = ""; //记录统计  (mysql要求必须添加 最后的as t)
        if("mysql".equals(dialect)){
        	countSql = "select count(1) from (" + sql+ ") as t"; //记录统计  (mysql要求必须添加 最后的as t)
	...
	private String generatePageSql(String sql, Page page){
        ...
        if("mysql".equals(dialect)){
            pageSql.append(sql);
            pageSql.append(" limit ").append(page.getOffset()).append(",").append(page.getLimit());  
        }
        else if("oracle".equals(dialect)){
            pageSql.append("select * from (select t.*, ROWNUM num from (")
            	.append(sql).append(") t where ROWNUM <= ")
	            .append(page.getOffset() + page.getLimit())
